...
const release: Release = {
  product: "claude", version: "2.1.267", displayVersion: "2.1.267", title: "Claude Code 2.1.267",
  notes: "- Added `maxEffortLevel` setting", url: "https://github.com/anthropics/claude-code/releases/tag/v2.1.267",
};
const post = validateChinesePost(`【Claude】Claude Code 2.1.267 发布\n\n• 新增 \`maxEffortLevel\` 设置，可按模型限制努力等级上限\n\n${release.url}`, release);
...
assert.deepEqual(blocks.map(block => block.version), ["1.0.13", "1.0.12", "1.0.11"]);
console.log("OK: complete Chinese payload, broken-draft rejection, X length and Grok version parsing");
